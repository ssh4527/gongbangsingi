...
<script type="text/javascript">
	function chk() {
		...
		var tel4 = document.getElementById('tel4'); // 사업자 등록번호1
		var tel5 = document.getElementById('tel5'); // 사업자 등록번호2
		var tel6 = document.getElementById('tel6'); // 사업자 등록번호3

		if (!chk(/^[가-힣]{2,}$/, name, "한글로 2글자 이상을 넣으세요~"))
			return false;
		if (!chk(/^[가-힣]{1,}$/, name2, "한글로 1글자 이상을 넣으세요~"))
			return false;
		...
		if (!chk(/^[0-9]{4}$/, tel3, "공방 전화번호3 : 4자리 번호 입력"))
			return false;
		if (!chk(/^[0-9]{3}$/, tel4, "사업자 등록번호1 : 3자리 번호 입력"))
			return false;
		if (!chk(/^[0-9]{2}$/, tel5, "사업자 등록번호2 : 2자리 번호 입력"))
			return false;
		if (!chk(/^[0-9]{5}$/, tel6, "사업자 등록번호3 : 5자리 번호 입력"))
			return false;
